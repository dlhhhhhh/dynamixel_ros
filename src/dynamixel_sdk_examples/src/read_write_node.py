...
portHandler = port_handler(DEVICENAME)
packetHandler = PacketHandler(PROTOCOL_VERSION)

def set_goal_pos_callback(data):
    print("Set Goal Position of ID %s = %s, ID %s = %s" % (data.id, data.position, data.id2, data.position2))
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, data.id, ADDR_GOAL_POSITION, data.position)
    dxl_comm_result2, dxl_error2 = packetHandler.write4ByteTxRx(portHandler, data.id2, ADDR_GOAL_POSITION, data.position2)
...
def get_present_pos(req):
    ...
    # dxl_present_position, dxl_comm_result2, dxl_error2 = packetHandler.read4ByteTxRx(portHandler, req.id2, ADDR_PRESENT_POSITION)
