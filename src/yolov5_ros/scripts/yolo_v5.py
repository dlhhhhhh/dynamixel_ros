...
class Yolo_Dect:
    ...
    def dectshow(self, org_img, boxs, height, width):
        ...
        for box in boxs:
            ...
            boundingBox.center_x = (boundingBox.xmin + boundingBox.xmax) / 2
            boundingBox.center_y = (boundingBox.ymin + boundingBox.ymax) / 2
    ...
    def publish_image(self, imgdata, height, width):
        ...
        for box in self.boundingBoxes.bounding_boxes:
            center_msg = Point()
            center_msg.x = (box.xmin + box.xmax) / 2
            center_msg.y = (box.ymin + box.ymax) / 2
            center_msg.z = 0.0  # 没有深度信息，所以z坐标设为0
...
def main():
    ...
    # center_pub = rospy.Publisher('/yolov5/center_coordinates', Point, queue_size=10)
    yolo_dect = Yolo_Dect()
    rospy.spin()
